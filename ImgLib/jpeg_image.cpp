...
bool SaveJPEG(const Path& file, const Image& image) {
    ...
    std::vector<JSAMPLE> row_buffer(
            static_cast<size_t>(image.GetWidth()) * cinfo.input_components
    );
    JSAMPROW row_pointer[1];
    row_pointer[0] = row_buffer.data();

    while (cinfo.next_scanline < cinfo.image_height) {
        const Color* src_line =
                image.GetLine(static_cast<int>(cinfo.next_scanline));

        for (int x = 0; x < image.GetWidth(); ++x) {
            row_buffer[x * 3 + 0] = static_cast<JSAMPLE>(src_line[x].r);
            row_buffer[x * 3 + 1] = static_cast<JSAMPLE>(src_line[x].g);
            row_buffer[x * 3 + 2] = static_cast<JSAMPLE>(src_line[x].b);
        }
